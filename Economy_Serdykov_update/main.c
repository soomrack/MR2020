...
int sal,srok,perv,nachk, stkv, perep,obshst, ostsrm,stkvip, srokd, dpd, schet, i, drop, mesyacbezedy,microprocent;
double prip,mprip,obshstav, mplip,prrkv, mind, mprd,prd;
int main() {
    printf("Your dohod.\n");
    scanf("%d",&sal);
    printf("Procent po microzaimu.\n");
    scanf("%d",&microprocent);
    printf("Kolychestvo mesyacev prosrotchki.\n");
    scanf("%d",&mesyacbezedy);
    printf("Drop ru'BLYA percent.\n");
    scanf("%d",&drop);
    printf("Stoimost kv.\n");
    scanf("%d",&stkv);
    ...
    stkvip=(prrkv/100*srok+1)*stkv*(1+(drop/100))-(mplip*mesyacbezedy*(microprocent/100));// В вывод
    mprd=prd/100/12;
    schet=nachk;
    while (i<=srokd*12) {
