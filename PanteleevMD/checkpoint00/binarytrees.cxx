...
Node* Search_min(Tree Tree) {
    Node* answer=Tree.Get_root(); 
    while(answer->left!=NULL) {         //move all the way to the left
        answer=answer->left;
    }
    printf("\n%d",answer->key);
    return answer;
...
Node* Search_max(Tree Tree) {
    Node** answer=Tree.Get_rooot(); 
    while((*answer)->right!=NULL) {     //move all the way to the right
        (*answer)=(*answer)->right;
    }
    printf("\n%d",(*answer)->key);
    return (*answer);
