...
const int tree_rank = 2;

class BNode
{
public:
    int keys[(2 * tree_rank - 1)];
    BNode *children[(2 * tree_rank)];
    int count;
    bool leaf;
    BNode(int key);
};
...
{
    this->keys[0] = key;
    for (int i = 1; i < (2 * tree_rank - 1); i++)
    {
        this->keys[i] = 0;
    }
    for (int i = 0; i < (2 * tree_rank); i++)
    {
...
private:
    ...
    void setting_new_children_for_node(BNode *new_child1,BNode *new_child2, BNode *node);
...
void Tree::add_data(int key)
{
    ...
    while (temp->count >= (2 * tree_rank - 1))
    {
        if (temp == root)
        {
            restruct(temp);
...
void Tree::restruct(BNode* node)
{
    ...
    new_child2->count = 0;
    setting_new_children_for_node(new_child1, new_child2, node);
    if (node == root)
    {
        node->keys[0] = node->keys[tree_rank-1];
        for (int i = 1; i < (2 * tree_rank - 1); i++)
        {
            node->keys[i] = 0;
        }
    ...
    else
    {
        ...
        insert(node->keys[tree_rank - 1], parent_node);
        ...
        for (int i = 0; i < (2 * tree_rank); i++)
        {
            ...
            {
                for (int j = (2 * tree_rank - 1); j > (i + 1); j--)
                {
...
void Tree::setting_new_children_for_node(BNode* new_child1, BNode* new_child2, BNode* node)
{
    for (int i = 0; i < (tree_rank - 1); i++)
    {
        new_child1->keys[i] = node->keys[i];
        new_child2->keys[i] = node->keys[i + tree_rank];
    }
    new_child1->count = tree_rank - 1;
    new_child2->count = tree_rank - 1;
    if (node->children[0] != nullptr)
    {
        for (int i = 0; i <= (tree_rank - 1); i++)
        {
            new_child1->children[i] = node->children[i];
            new_child2->children[i] = node->children[i + tree_rank];
        }
...
void Tree::delete_key(int key)
{
    ...
    if ((node_for_delete->leaf = true) && (node_for_delete->count = tree_rank))
    {
...
{
    Tree test_tree = Tree();
    test_tree.add_data(6);
    test_tree.print_tree(test_tree.root);
    test_tree.add_data(8);
    test_tree.print_tree(test_tree.root);
    test_tree.add_data(10);
    test_tree.print_tree(test_tree.root);
    test_tree.add_data(4);
    test_tree.print_tree(test_tree.root);
    test_tree.add_data(12);
    test_tree.print_tree(test_tree.root);
    test_tree.add_data(14);
    test_tree.print_tree(test_tree.root);
