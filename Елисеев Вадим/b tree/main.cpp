...
class Tree { //создадим класс Tree, включающий в себя соответствующие методы
    private:
    BNode *root;
    void insert_to_node(int key, BNode *node);	// Метод простого добавления
    void sort(BNode *node);						// Метод сортировки чисел в узле
    void restruct(BNode *node);					// Метод разбиения узла
    void deletenode(BNode *node);
    bool searchKey(int key, BNode *node);	
    void remove(int key, BNode *node);			// Метод удаления из произвольного узла
    void removeFromNode(int key, BNode *node); // Метод удаления ключа из узла
    void removeLeaf(int key, BNode *node);		// Метод удаления ключа из листа
    void lconnect(BNode *node, BNode *othernode); // Методы соединения узлов
    ...
    Tree();
    ~Tree();
    void insert(int key); // Метод добавления значения в узел с предварительным поиском позиции
    bool search(int key);
    void remove(int key); // Основной метод удаления
};
...
void Tree::insert_to_node(int key, BNode *node){ 
    node->keys[node->count]=key;		//В данном методе вызывается метод сортировки, 
    node->count=node->count+1;			//необходимый для выполнения условия о возрастающих значениях дерева
    sort(node);
}
...
//---В данном методе дерево в буквальном смысле перестраивается до тех пор, пока не будут удовлетворены все условия B-дерева---//								
...
//---принимает в качестве входных данных — значение ключа для удаления из дерева---//
    BNode *ptr=this->root;
    int position;
    int positionSon;
    int i;
