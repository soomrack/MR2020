...
int insert_move(char symbol, int row, int col){

    if ((row < 0) || (row > 2) || (col < 0) || (col > 2)) {return 2;}

    if (table[row][col] != '-'){ return 1; }
...
void make_turn(void){
    ...
    for (int i = 0; i < 3; i++){
        if ((table[i][0] == table[i][2]) && (table[i][0] != '-') && (table[i][1] == '-')){
            insert_move('O', i, 1); 
            return;
        }
        ...
        if ((table[0][i] == table[2][i]) && (table[0][i] != '-') && (table[0][i] == '-')){
            insert_move('O', 1, i); 
            return;
        }
    ...
    for (int i = 0; i < 3; i++){
        for (int j = 0; j < 3; j++){
            if (table[i][j] == '-') { insert_move('O', i, j); return; }
        }
    }
...
void play_game(void){
    ...
    while (! is_ended){
        ...
        if (insert_move('X', index[0] - 48 - 1, index[1] - 48 - 1) == 1) {
            printf("Enter there is already a symbol");
            continue;
        }

        if (insert_move('X', index[0] - 48 - 1, index[1] - 48 - 1) == 2) {
